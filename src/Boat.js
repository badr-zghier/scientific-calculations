...
class Boat extends Grid {
  ...
  // زاوية دوران الشراع
    ...
    applyThrottleVelocity() {
        ...
        const throttleAcceleration = throttleDirectionVec3.multiplyScalar(throttleScale);
        this.applyAcceleration(throttleAcceleration);
        this.acceleration.copy(throttleAcceleration);
    }

    updateBob(t) {
        const waveHeight = parseFloat(document.getElementById('waveHeightInput').value);
    ...
    calculateThrustForce() {
        const windSpeed = parseFloat(document.getElementById('windSpeedInput').value) || 5; // قيمة افتراضية
        const sailArea = parseFloat(document.getElementById('sailAreaInput').value) || 10; // قيمة افتراضية
    ...
    calculateDragForce() {
        ...
        return 0.5 * dragCoefficient * densityOfWater * this.wetArea * velocitySquared;
    }

    calculateWaterMomentumAndKineticEnergy() {
        const waterSpeed = parseFloat(document.getElementById('waterSpeedInput').value);
    ...
    calculateElasticCollisionVelocity() {
        ...
        // حفظ كمية الحركة
        const totalMomentum = (waterMass * waterSpeed) + (boatMass * this.velocity.length());
    
        // حفظ الطاقة الحركية
        const totalKineticEnergy = (0.5 * waterMass * waterSpeed * waterSpeed) + (0.5 * boatMass * this.velocity.length() * this.velocity.length());
    ...
    updateInfoPanel(buoyantForce, gravityForce, waveForce) {
        document.getElementById('speedDisplay').innerText = this.velocity.length().toFixed(2);
        document.getElementById('accelerationDisplay').innerText = this.acceleration.length().toFixed(2);
        document.getElementById('rotationDisplay').innerText = this.rotation.y.toFixed(2);
        document.getElementById('buoyantForceDisplay').innerText = buoyantForce.toFixed(2);
        document.getElementById('gravityForceDisplay').innerText = gravityForce.toFixed(2);
        document.getElementById('waveForceDisplay').innerText = waveForce.toFixed(2);
        document.getElementById('momentumDisplay').innerText = this.calculateMomentum().toFixed(2);
        document.getElementById('angularSpeedDisplay').innerText = this.angularVelocity.length().toFixed(2);
        
        // عرض الطاقة الحركية على الشاشة
        const kineticEnergy = this.calculateKineticEnergy();
        document.getElementById('kineticEnergyDisplay').innerText = kineticEnergy.toFixed(2);
    
// حساب العزم الناتج عن الرياح وعرضه على الشاشة
const windTorque = this.calculateWindTorque();
document.getElementById('windTorqueDisplay').innerText = windTorque.toFixed(2);

// عرض قوة الدفع على الشاشة
const thrustForce =  this.calculateDragForce() + this.calculateThrustForce();
document.getElementById('thrustForceDisplay').innerText = thrustForce.toFixed(2);

        // عرض قوة السحب على الشاشة
        const dragForce = this.calculateDragForce();
        document.getElementById('dragForceDisplay').innerText = dragForce.toFixed(2);

// حساب وعرض كمية الحركة والطاقة الحركية للماء
const { waterMomentum, waterKineticEnergy } = this.calculateWaterMomentumAndKineticEnergy();
    document.getElementById('waterMomentumDisplay').innerText = waterMomentum.toFixed(2);
    document.getElementById('waterKineticEnergyDisplay').innerText = waterKineticEnergy.toFixed(2);

document.getElementById('momentumDisplay').innerText = waterMomentum.toFixed(2) ;
document.getElementById('kineticEnergyDisplay').innerText = waterKineticEnergy.toFixed(2) + " (J)";
     ...
     this.updateSpeedometer(this.velocity.length());
        // عرض اتجاه البوصلة
        document.getElementById('compassDirectionDisplay').innerText = this.getCompassDirection();
    }
...
adjustSpeedBasedOnWind() {
    ...
    if (windIndex === boatIndex) {
        ...
        this.velocity.add(this.velocity.clone().normalize().multiplyScalar(windSpeed/20));
    } else {
...
update(t, now) {
    ...
    const dragForce = this.calculateDragForce();
    this.applyForce(new THREE.Vector3(-dragForce * this.velocity.x / this.velocity.length(), 0, -dragForce * this.velocity.z / this.velocity.length()));
    ...
    this.velocity.add(this.velocity.clone().normalize().multiplyScalar(thrustForce * t));
